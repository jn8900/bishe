...
<table style="position: absolute; font: '华文新魏'; font-size: 36px; left: 459px; top: 229px; color:#903;">
...
product_bb=new String(request.getParameter("product_bb").getBytes("ISO8859_1"));
product_sn=new String(request.getParameter("product_sn").getBytes("ISO8859_1"));
product_price=Integer.parseInt(request.getParameter("product_price"));
product_discount=Integer.parseInt(request.getParameter("product_discount"));
product_type=new String(request.getParameter("product_type").getBytes("ISO8859_1"));


 //将用户信息存入数据库

String sql="select * from mjxxlist order by product_id desc";
ResultSet rs=mjxxbean.executeQuery(sql);
 if(rs.next())
 {
 
	product_id=rs.getInt(1)+1;//最后一条记录的id加1
}
sql="insert into mjxxlist(product_id,product_name,product_bb,product_sn,product_price,product_discount,product_type) values("+product_id+",'"+product_name+"','"+product_bb+"','"+product_sn+"',"+product_price+","+product_discount+",'"+product_type+"')";    
mjxxbean.executeUpdate(sql);
 

 %>
<div align="center" class="one">
	<p>服装<%=product_name%>已添加成功</p>
</div>
